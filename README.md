...
**Built-in Web Management Panel** — A browser-based admin UI with token authentication, IP rate limiting, and security headers. Create, view, and delete codes; browse operation logs; check server status; view 7-day redemption trends; export CSV — all from your browser.
...
**Async I/O Architecture** — All blocking I/O (SQL, Redis, file, web) runs on a dedicated thread pool, ensuring the Minecraft server thread is never blocked. Web handlers that read game state (players, scoreboards) dispatch to the main thread via `CompletableFuture` for thread safety.
...
- **Admin Dashboard** (custom path, auto-generated on first run) — Full management interface including:
  ...
  - Hot reload button
  - 7-day redemption trend chart and CSV export

**Security features:**

- Token-based session authentication with 30-minute sliding TTL (single-session enforcement: new login invalidates previous)
- IP rate limiting: login endpoint 5 requests/min, API endpoints 20 requests/sec
- Security response headers (X-Frame-Options, X-Content-Type-Options, CSP)
- Optional independent Redis backend (`web.redis.*`) for token/rate-limit state sharing across restarts; automatic in-memory fallback when unavailable
...
# ── Web Security Redis (optional) ─────────────────────
# Independent Redis for token storage & rate limiting.
# When disabled, falls back to in-memory mode automatically.
web.redis.enabled=false
web.redis.host=localhost
web.redis.port=6379
web.redis.password=
