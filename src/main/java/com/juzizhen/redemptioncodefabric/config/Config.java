package com.juzizhen.redemptioncodefabric.config;
...
public class Config {
    ...
    private void saveOrganized() throws IOException {
        try (BufferedWriter w = new BufferedWriter(new FileWriter(CONFIG_FILE))) {
            ...
            prop(w, "web.redis.password");
            prop(w, "web.redis.database");
    ...
    private boolean ensureAllKeysExist() {
        ...
        updated |= maybeSetDefault("web.redis.password", "");
        updated |= maybeSetDefault("web.redis.database", "0");
